...
def editar_aula(request):
    ...
    if request.POST.get('descricao', False):
        for v in codigos:
            if request.POST['descricao'] != '' and \
                request.POST['descricao'] in v['desc_habilidade']:
                escolha = {
                    'cod_hab': v['cod_hab'],
    ...
    if aula.cod_hab.cod_hab != hab or \
        aula.fluxo_aula != fluxo or \
        aula.info_adicionais != info:

        aula.cod_hab = Habilidade.objects.get(cod_hab=hab) if aula.cod_hab != hab else aula.cod_hab
        aula.desc_aula = Habilidade.objects.get(cod_hab=hab).desc_habilidade if aula.cod_hab != hab else aula.desc_aula
        aula.fluxo_aula = fluxo if aula.fluxo_aula != fluxo else aula.fluxo_aula
        aula.info_adicionais = info if aula.info_adicionais != info else aula.info_adicionais
        aula.save()
    ...
    return render(request, template_name='minhas-aulas/editar-aula.html', context=context)